...
export function no_person_type() {
    var url = data.url;
    var params = {
      headers: {
    ...
    check(res, { 'status was 400 no person_type' : (r) => r.status == 400 });

    check(res, { 'error code was parameter_required' : (r) => JSON.parse(r.body).Errors[0].Code == "parameter_required"});
    // message
    check(res, { 'error message was person_type is required' : (r) => JSON.parse(r.body).Errors[0].Message == "The field person_type is required" });
    // params
    check(res, { 'error params was person_type in index 0' : (r) => JSON.parse(r.body).Errors[0].Params[0] == "person_type" });
    // detail
